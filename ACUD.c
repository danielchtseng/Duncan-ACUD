...
// Declare Event related to  		
#defie 	UART_RxComp		0x01;
...
int 	UART_Inbuf_Index;
int 	UART_Outbuf_Index;
char 	UART_Outbuf[UART_Inbuf_Max];
char 	UART_Iutbuf[UART_Outbuf_Max];
char 	Event
...
void UART_Init(float Fosc ,int Baudrate){	
	...
	*/
	...
	SCON=0X40;		// SCON=01000000B, SMOD 1 ,8 bit data and 1 stop bit, 
...
void UART_ISR() interrupt 4 {
	...
	if ( RI ){ 			// SCON.RI, RI=1 means new content have received                       	
		...
		if( UART_Inbuf_Index = UART_Inbuf_Max {
			...
			if (Event=0) {
				Event=UART_RxComp;			// Updating Event for ACUD_StateEvent()
				UART_Inbuf_Index=0;			// Reset UART_Inbuf_Index
		
			else {		// ***** Triger UART_Rx interrupt again
				RI=0; 
				N_msDelay(1);
				RI=1;
			}
	...
	if ( TI ){    		// SCON.TI, TI=1 means previous content have been sent.   
		...
		if ( UART_Outbuf_Index = UART_Outbuf_Max ){
			...
			if (Event=0) {
				Event=UART_TxComp;			// Updating Event for ACUD_StateEvent()
...
UART_Send(){
	...
	UART_Outbuf_Index=0x00;
	TI = 1;
	
}

	
void N_msDelay(int N){	//	Delay t*1ms 
	
	int i,j,N;				//	宣告整數變數i,j,N 
